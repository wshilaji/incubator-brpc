...
namespace base {
namespace impl {
using IntDeleter = ::base::ShardCache::IntDeleter;

// Cache entry meta data.
struct IntClockCacheHandle {
    IntKeyType key;
    void *value;
    size_t charge;
    IntDeleter deleter;
    ...
    IntClockCacheHandle() = default;
    IntClockCacheHandle(const IntClockCacheHandle &a) {
        *this = a;
    }
    IntClockCacheHandle(const IntKeyType &k, void *v, IntDeleter del)
        : key(k), value(v), deleter(del) {}

    IntClockCacheHandle &operator=(const IntClockCacheHandle &a) {
...
struct IntCleanupContext {
    // List of values to be deleted, along with the key and deleter.
    std::vector<IntClockCacheHandle> to_delete_value;

    // List of keys to be deleted.
    std::vector<uint64_t> to_delete_key;
};

// A cache shard which maintains its own CLOCK cache.
class IntClockCacheShard {
public:
    // Hash map type.
    using CacheHandle = IntClockCacheHandle;
    using HashTable = tbb::concurrent_hash_map<IntKeyType, CacheHandle *, IntKeyType>;
    ...
    bool insert(const IntKeyType &key, void *value, size_t charge,
                IntDeleter deleter, CacheHandle **out_handle);
    CacheHandle *lookup(const IntKeyType &key);
    ...
    void erase(const IntKeyType &key);
    bool eraseAndConfirm(const IntKeyType &key, IntCleanupContext *context);
...
private:
    ...
    //
    // Not necessary to hold mutex_ before being called.
    bool unref(CacheHandle *handle, bool set_usage, IntCleanupContext *context);
    ...
    bool unsetInCache(CacheHandle *handle, IntCleanupContext *context);
    ...
    //
    // Has to hold mutex_ before being called.
    bool tryEvict(CacheHandle *value, IntCleanupContext *context);
    ...
    //
    // Has to hold mutex_ before being called.
    bool evictFromCache(size_t charge, IntCleanupContext *context);

    CacheHandle *insert(
        const IntKeyType &key, void *value, size_t charge,
        IntDeleter deleter, bool hold_reference, IntCleanupContext *context);
...
IntClockCacheShard::IntClockCacheShard()
...
void IntClockCacheShard::recycleHandle(CacheHandle *handle, IntCleanupContext *context) {
...
void IntClockCacheShard::cleanup(const IntCleanupContext &context) {
...
bool IntClockCacheShard::ref(CacheHandle *handle) {
...
bool IntClockCacheShard::unref(CacheHandle *handle, bool set_usage, IntCleanupContext *context) {
    if (set_usage) {
...
bool IntClockCacheShard::unsetInCache(CacheHandle *handle, IntCleanupContext *context) {
    // Use acquire-release semantics as previous operations on the cache entry
    // has to be order before reference count is decreased, and potential cleanup
    // of the entry has to be order after.
...
bool IntClockCacheShard::tryEvict(CacheHandle *handle, IntCleanupContext *context) {
...
bool IntClockCacheShard::evictFromCache(size_t charge, IntCleanupContext *context) {
...
void IntClockCacheShard::setCapacity(size_t capacity) {
    IntCleanupContext context;
    {
        std::lock_guard<std::mutex> l(mutex_);
...
void IntClockCacheShard::setStrictCapacityLimit(bool strict_capacity_limit) {
    strict_capacity_limit_.store(strict_capacity_limit, std::memory_order_relaxed);
}
IntClockCacheShard::CacheHandle *IntClockCacheShard::insert(
    const IntKeyType &key, void *value, size_t charge,
    IntDeleter deleter, bool hold_reference, IntCleanupContext *context) {
    std::lock_guard<std::mutex> l(mutex_);
...
bool IntClockCacheShard::insert(const IntKeyType &key, void *value, size_t charge,
                             IntDeleter deleter, CacheHandle **out_handle) {
    IntCleanupContext context;
    HashTable::accessor accessor;
    ...
    // IntKeyType key_copy{slice, key.hash_value};
    CacheHandle *handle = insert(key, value, charge, deleter,
...
IntClockCacheShard::CacheHandle *IntClockCacheShard::lookup(const IntKeyType &key) {
    ...
    if (!IntKeyType::equal(handle->key, key)) {
        IntCleanupContext context;
...
bool IntClockCacheShard::release(CacheHandle *handle, bool force_erase) {
    IntCleanupContext context;
...
void IntClockCacheShard::erase(const IntKeyType &key) {
    IntCleanupContext context;
    eraseAndConfirm(key, &context);
    cleanup(context);
}
bool IntClockCacheShard::eraseAndConfirm(const IntKeyType &key, IntCleanupContext *context) {
    std::lock_guard<std::mutex> l(mutex_);
    HashTable::accessor accessor;
    bool erased = false;
    if (table_.find(accessor, key)) {
...
void IntClockCacheShard::eraseUnRefEntries() {
    IntCleanupContext context;
    {
        std::lock_guard<std::mutex> l(mutex_);
        table_.clear();
...
std::shared_ptr<ShardCache> NewIntClockCache(
    ...
    return std::make_shared<::base::impl::ShardCache<IntKeyType, impl::IntClockCacheShard>>(
