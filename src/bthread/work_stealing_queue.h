 ...
 * */
 ...
 /*
一个 bthread 在 RPC 中遇到需要等待的情况（如下游服务未返回）时，会触发调度，这个过程主要涉及以下几点：
1   主动让出：调用 sched 或 yield：当 bthread 需要等待时，会调用 TaskGroup::sched 或 bthread_yield 这样的函数。
这相当于 bthread 主动通知调度器：“我现在需要等待，请把 CPU 让给其他任务。”
2   上下文切换：保存与恢复：主动让出触发了一次上下文切换。当前 bthread 的执行上下文（如寄存器状态、栈指针）会被保存起来。
然后，TaskGroup（任务组）会从它的运行队列中取出下一个就绪的 bthread，并恢复它的上下文，使其开始运行。
3   放入等待队列：使用 butex 同步原语：关键点在于，bthread 在让出 CPU 之前，会将自己放入一个“等待队列”中。
这个队列的实现基于 brpc 的 butex（bthread mutex）同步原语。当一个 bthread 因等待锁或条件变量而阻塞时，
它其实是在 butex 的等待队列上挂起。
4   工作窃取（Work Stealing）与空闲处理：当一个 pthread 工人线程发现它的本地运行队列为空时，它不会立即进入睡眠。
它会尝试从其他工人线程的队列中“窃取”任务来执行。如果所有队列都为空，工人线程才会进入睡眠状态，等待新任务（或被唤醒的等待任务）的到来
  */